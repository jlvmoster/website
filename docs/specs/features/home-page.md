...
## Goal
The `/` route. Hero (verbatim copy + three social links) + Photos strip + 4 most-recent article cards + Resume timeline + Download CV.
...
## File layout
- `src/pages/HomePage.tsx` — composes Hero JSX inline, Photos, ArticleCard grid, Resume.
- `src/components/home/Photos.tsx` — five rotated photos in a flex row.
...
- `public/images/avatar.jpg`, `public/images/photos/image-{1..5}.jpg`, `public/images/logos/*.svg`, `public/cv.pdf`.
...
- **E2E**: Photos strip renders 5 elements; Resume renders ≥ 1 row with the canonical job entries; "Download CV" link resolves to `/cv.pdf`.
...
- Photos (5 images). User to provide.
