...
- **FR-1.2.5 Home page** — `/` renders Hero (per §FR-1.2.1.a/b) + a Photos strip (five rotated images) + a two-column body (left: 4 most-recent article cards; right: Resume timeline with a "Download CV" button). No newsletter signup.
...
- **FR-1.3.2** Design tokens are exposed as CSS variables: `--bg`, `--fg`, `--muted`, `--accent`, `--font-sans`, `--font-serif`.
  - Token values map to the zinc + teal palette: `--bg` = zinc-50 / zinc-950, `--fg` = zinc-900 / zinc-200, `--muted` = zinc-600 / zinc-400, `--accent` = teal-500 / teal-400. Two additional tokens model the Spotlight panel chrome — see §FR-1.3.6.
...
- **FR-1.5.7** `scripts/build.ts` and `scripts/dev.ts` continue to copy `public/` → `dist/` so `public/images/{avatar,portrait}.jpg`, `public/images/photos/`, `public/images/logos/`, and `public/cv.pdf` ride along to production.
...
| Color palette | Zinc + teal (Tailwind palette) |
