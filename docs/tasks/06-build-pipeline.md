...
3. **Create `public/`** at repo root and seed it with:
   ...
   - `og-image.png` — 1200x630 OG card. Acceptable to defer; if deferred, leave the `<meta property="og:image">` tag in `index.html` but document the gap in Task 08 acceptance.
...
- `bun run preview` (`wrangler dev`, defined Task 01) serves the built `dist/` through the Workers runtime.
